...
#view customer address by entering the order_id
select cust_addr from customer where cust_id=(select fk_cust_id from orders where order_id=1);

#view item details by entering the order_id
select item_name from items where item_id=(select fk_item_id from order_items where fk_order_id=2 limit 1); 

#use customers name to get their order id
select order_id from orders where fk_cust_id=(select cust_id from customer where cust_name='mike' limit 1);

select * from order_items;

#trying to make the orderline_total update to the price of the item multiplied by its quanitity. 
#find out the details from each order item
select price from items where item_id=2; #10
select quantity from order_items where oi_id=1; #5